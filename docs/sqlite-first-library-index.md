...
| Material texture slots | `ue_source_index.db.material_texture_slots`, optionally `library_work.db.material_texture_slots`, then `library_index.db.material_texture_slots` | `material_texture_slots.jsonl` is a debug/human view |
| Shared glTF texture rewrites | `library_work.db.shared_gltf_texture_links`, then `library_index.db.shared_gltf_texture_links` | `shared_texture_gltf_links.jsonl` is a debug/human view |
| UE source index metadata | `ue_source_index.db.source_index_metadata` | `ue_source_index.metadata.json` is a legacy debug/human view |
| UE source relations | `ue_source_index.db`, optionally `library_work.db.component_asset_relations`, then `library_index.db.component_asset_relations` | `component_asset_relations.jsonl` is a debug/human view |
| Component groups | `library_index.db.component_groups` for owner-level counts; full details in `library_index.db.component_asset_relations` | `component_groups.json` is a debug/human view |
| UE package object maps | `ue_source_index.db.package_object_maps`, then optional sampled rows in `library_index.db.package_object_maps` | `package_object_maps.jsonl` is a debug/human view |
...
| Model validation | `library_index.db.model_validation` and `library_index.db.model_validation_notes` | `model_validation.json` is a debug/human view |
| Model coverage | `library_index.db.model_coverage` | `model_coverage.json` is a debug/human view |
| Task model quality | `library_index.db.model_coverage`, `library_index.db.model_coverage_task_signals` and `library_index.db.model_coverage_review_reasons` | `task_model_quality.json` is a debug/human view |
| Skeleton groups | `library_index.db.skeleton_groups`, `skeleton_group_paths`, `skeleton_group_source_objects`, `skeleton_group_models`, `skeleton_group_animations` and `skeleton_group_bone_names` | `skeletons.json` is a debug/human view |
| Animation validation | `library_index.db.animation_validation`, `library_index.db.animation_validation_missing_track_bones` and `library_index.db.animation_validation_hierarchy_mismatches` | `animation_validation.json` and `animation_validation.jsonl` are debug/human views; validation facts are explicit columns/child rows |
| Browser model/animation list | `library_index.db.model_animation_relations`, `library_index.db.relation_animations`, `relation_animation_evidence`, `relation_animation_segments` and `relation_animation_sections` | Browser must not depend on `model_animations.json`; model relation summaries are explicit columns, per-animation rows live in `relation_animations`, ordered evidence steps live in `relation_animation_evidence`, and container segment/section details live in child tables |
...
| Library health and acceptance reports | `library_index.db.library_reports` | `library_health.json` and `library_acceptance.json` are debug/human views |
...
- Auto referenced export diagnostics now stream to `export_events.db.auto_referenced_exports` in bounded batches during rule planning; `auto_referenced_exports.jsonl` is only written when debug JSON is explicitly enabled.
- `--materialize-animation-metadata` updates `export_events.db.asset_catalog` and `export_events.db.animation_bindings` by default; `.ueanim.metadata.json`, `asset_catalog.jsonl` and `animation_bindings.jsonl` are only written or updated when `--debug-json` is explicitly requested.
...
- Postprocess skips animation validation and model-animation JSON views by default; query `animation_validation`, `animation_validation_missing_track_bones`, `animation_validation_hierarchy_mismatches`, `model_animation_relations` and `relation_animations` in `library_index.db`. `animation_validation` stores `track_source` as an explicit column and no longer keeps a nested `raw_json` copy.
- Postprocess skips model validation and skeleton JSON views by default; query `model_validation`, `model_validation_notes`, `skeleton_groups`, `skeleton_group_paths`, `skeleton_group_source_objects`, `skeleton_group_models`, `skeleton_group_animations` and `skeleton_group_bone_names` in `library_index.db`. Model bounding boxes are explicit numeric columns in `model_validation`, while validation notes are rows in `model_validation_notes`. Skeleton groups no longer keep a nested `raw_json` blob; paths, source objects, grouped models, grouped animations and sampled bone names are normalized into child tables.
...
- Source-index resume metadata now lives in `ue_source_index.db.source_index_metadata`; `ue_source_index.metadata.json` is only written when debug JSON is explicitly enabled.
