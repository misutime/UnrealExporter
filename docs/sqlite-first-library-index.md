...
| Skeleton groups | `library_index.db.skeleton_groups`, `skeleton_group_paths`, `skeleton_group_source_objects`, `skeleton_group_models`, `skeleton_group_animations` and `skeleton_group_bone_names` | `skeletons.json` is a compatibility/debug view |
| Animation validation | `library_index.db.animation_validation`, `library_index.db.animation_validation_missing_track_bones` and `library_index.db.animation_validation_hierarchy_mismatches` | `animation_validation.json` and `animation_validation.jsonl` are compatibility/debug views; validation facts are explicit columns/child rows |
