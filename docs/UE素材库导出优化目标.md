...
- 已支持 `autoExportReferencedAssets`，在源索引完成后按显式组件/蓝图 PPtr 自动补导被引用的 StaticMesh、SkeletalMesh、Material、Texture、Animation 和 AnimBlueprint JSON，避免任务道具只导出主模型而漏掉组合部件。
