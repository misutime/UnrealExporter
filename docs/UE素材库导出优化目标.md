...
- 大库默认就是 SQLite-first，避免机器索引重复写成超大 JSONL 后再读回内存；验收脚本和浏览器必须以 SQLite 表为准。
