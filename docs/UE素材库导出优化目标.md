...
6. 索引与报告
   ...
   - 即使 `generateLibraryIndexes=false` 只运行共享贴图去重，`sqliteOnlyIndex` / `writeCompatibilityJson:false` 也必须生效：贴图链接和去重摘要写入 `library_work.db`，不额外生成 `texture_links.jsonl` / `texture_dedupe_summary.json`。
