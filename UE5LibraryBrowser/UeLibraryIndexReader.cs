        ...
        Dictionary<string, List<UeLibraryAnimation>> animationsByModel)
    {
        ...
        command.CommandText = """
            SELECT a.name,
                   ...
                   a.source_type,
                   a.resource_kind,
                   COALESCE(a.skeleton_path, ''),
                   COALESCE(a.skeleton_name, ''),
                   COALESCE(mv.bone_count, 0),
                   COALESCE(mc.material_count, mv.material_count, 0),
                   COALESCE(mc.has_skin, CASE WHEN mv.skin_count > 0 THEN 1 ELSE 0 END, 0),
