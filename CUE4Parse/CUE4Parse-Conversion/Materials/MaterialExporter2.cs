        ...
        public override bool TryWriteToDir(DirectoryInfo baseDirectory, out string label, out string savedFilePath)
        {
            ...
            Parallel.ForEach(_materialData.Parameters.Textures.Values, texture =>
            {
                ...
                try
                {
                    ...
                    lock (FileWriteLock)
                    {
                        using var fs = new FileStream(texturePath, FileMode.Create, FileAccess.Write, FileShare.Read);
                        fs.Write(imageData, 0, imageData.Length);
                    }
                }
                catch (IOException)
                {
                    // Full exports may hit the same shared texture from several materials at once.
                    // The sidecar texture is best-effort because the GLB already embeds preview textures.
                }
            });
